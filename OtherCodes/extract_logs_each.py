...
exp_file = 'exp.txt'
# 包含实验数据的文件
files = ['exp1', 'exp2', 'exp3', 'exp4', 'exp5']
file_ext = '.txt'
interval = timedelta(milliseconds=300)  # ms
dt_format_short = '%Y-%m-%dT%H:%M'
dt_format_medium = '%Y-%m-%dT%H:%M:%S'
...
for file in files:
    ...
    with open(filename, 'r') as fp:
        for idx, line in enumerate(fp.readlines()):
            ...
            if len(items[1]) == 16:
                str_format = dt_format_short
            elif len(items[1]) == 19:
                str_format = dt_format_medium
            else:
                str_format = dt_format_long
            ...
            if cur_time < end_time:
                temp_dict[items[3]] = ','.join(
                    [end_time.strftime(dt_format_long), items[4], items[5], items[6],
